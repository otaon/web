...
## 12.6 マッピング
高階関数とは、関数を引数にとったり、関数を戻り値として返すような関数のことをいう。  
前の章の`map`なども高階関数だ。

`map`は任意のシーケンス(=リストとベクタの両方)に対して適用できる。  
`map`の他にも、リスト専用の高階関数が6つ存在する。

### `mapcar`
`mapcar`は`map`と似ているが、必ずリストを返す仕様。  
すなわち、`map`では第1引数に戻り値の型を表すシンボルを指定していたが、それが不要になる。
...
### `maplist`
`maplist`は`mapcar`に似ているが、関数の引数にするのが、リストの要素ではなくて、コンスセル。  
すなわち、リスト中で現在フォーカスしている要素と、それ以降の要素にもアクセスできる。

### `mapcan`
`mapcan`は`mapcar`のリサイクル版。  
リストの要素をマップされる関数の引数にする。  
そして、関数呼び出しの結果(リスト)を、`mapcan`の繰り返しごとに`nconc`のように繋ぎ合わせていく。

`remove`のようなフィルタリング操作用の関数がないLispの方言の場合、下記のように`mapcan`を使用することになる。
...
### `mapcon`
`mapcan`は`maplist`のリサイクル版。  
コンスセルをマップされる関数の引数にする。  
すなわち、リスト中で現在フォーカスしている要素と、それ以降の要素にもアクセスできる。  
そして、関数呼び出しの結果(リスト)を、`mapcon`の繰り返しごとに`nconc`のように繋ぎ合わせていく。

### `maplist` `mapcar` `mapcon` `mapcan` のまとめ
それぞれの関数の関係性を表にすると下記のようになる。
...
|----------------|----------------|-----------------------|
|**非破壊版**    |&nbsp;`mapcar`  |&nbsp;`maplist`        |
|**リサイクル版**|&nbsp;`mapcan`  |&nbsp;`mapcon`         |

----

下図のリスト`(1 2 3 4)`に対して、それぞれの関数は、その関数の色と同じ色の要素について走査する。  
リサイクル版の関数は、各関数呼び出しの結果を前回までの結果につなげていく。  

{{<figure src="map.svg" alt="litst" width="600" align="aligncenter">}}


### `mapc`と`mapl`
これらは、`mapc`と`mapl`の引数に渡したリストをそのまま返す。すなわち、これらは関数ではなく制御構文。

`mapc`は関数の引数に要素を渡す(`mapcar`や`mapcan`と同じ)。  
`mapl`は関数の引数にコンスセルを渡す(`maplist`や`mapcon`と同じ)。  


## 12.7 その他のデータ構造
次の章では、コンスセルを、木、集合、連想リストとして使用するための関数について説明する。
